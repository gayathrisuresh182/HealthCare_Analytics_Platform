#!/usr/bin/env python3
"""
Auto-configure Snowflake Datasource from dbt profiles
...
def main():
    ...
    if not creds or not all([creds.get('account'), creds.get('user'), creds.get('password')]):
        ...
        print("\nPlease run the interactive version:")
        print("  python scripts/gx_configure_snowflake_simple.py")
        sys.exit(1)
    ...
    try:
        ...
        # Use fluent API to add Snowflake datasource
